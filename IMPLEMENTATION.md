...
| ~~`arr[i] += val`~~ (compound indexed update) | ❌ | ❌ | **Withdrawn in v0.0.9 with the plain form.** All seven operators (`=` `+=` `-=` `*=` `/=` `%=` `^=`) are refused at the same site (`zymbol-parser/src/variables.rs:87`) — the rule is about `=` reaching inside a structure, and a compound `=` reaches the same way. Use `arr[i]$~ (arr[i] + val)` |
...
| Dictionaries `#(k: v)` | ✅ | ✅ | v0.0.9 gave them a notation of their own; the bare `(a: 1)` is refused |
...
| Destructuring assignment | ✅ | ✅ | Three patterns, all v0.0.2: `[a, b] = arr` (array), `(a, b) = t` (positional tuple), `#(name: n) = d` (dictionary). `_` discards a position in **both** the array and the tuple pattern (v0.0.9 — it was array-only) |
...
Source file: [`zymbol-lang.ebnf`](zymbol-lang.ebnf) — version 3.3.0, sprint v0.0.7 with three v0.0.9 correction passes, the last of which probed every rule it touched against the installed binary.

The canonical grammar is maintained in `zymbol-lang.ebnf`; the copy below is reproduced
**verbatim** from that file (synced 2026-09-07). If they ever diverge, the file wins.
